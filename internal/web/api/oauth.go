...
func (h *OAuthHandler) Authorize(w http.ResponseWriter, r *http.Request) error {
	...
	user, err := h.DB.GetUserByID(ctx, sess.UserID.Val)
	if err != nil {
		...
		h.Logger.Error("Failed to get user", "error", err, "user_id", sess.UserID.Val)
...
func (h *OAuthHandler) OAuthToken(w http.ResponseWriter, r *http.Request) error {
	ctx := r.Context()
	...
	if grantTypeRaw == "refresh_token" {
		...
		if refreshToken.UsedAt.IsSet {
			...
			if time.Since(refreshToken.UsedAt.Val) > time.Minute {
