...
func NewAPIHandler(logger *slog.Logger, db *database.Database, userManager *user.Manager) *APIHandler {
	return &APIHandler{
		Logger:      logger,
		DB:          db,
...
func (h *APIHandler) Healthy(w http.ResponseWriter, r *http.Request) error {
	...
	if err := h.DB.Ping(r.Context()); err != nil {
		...
		return ErrorResponse(w, http.StatusInternalServerError, http.StatusInternalServerError, "Database connection failed")
	}
