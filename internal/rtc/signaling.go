...
func (s *SignalingServer) HandleConnection(c *websocket.Conn, userID string) {
	// Register connection
	s.connMutex.Lock()
...
func (s *SignalingServer) handleSignalingMessage(c *websocket.Conn, msg []byte) {
	...
	s.logger.Info("Processing signaling message", "type", signal.Type, "from", fromUserID, "to", signal.To, "room", signal.RoomID)
...
func (s *SignalingServer) handleJoinRoom(c *websocket.Conn, roomID, userID string) {
	...
	s.logger.Info("User joining room", "user_id", userID, "room_id", roomID)
	...
	if !exists {
		...
		s.rooms[roomID] = room
		s.logger.Info("Created new room", "room_id", roomID)
	}

	// Add user to room
	room.mutex.Lock()
	room.Participants[userID] = c
	s.logger.Info("User added to room", "user_id", userID, "room_id", roomID, "total_participants", len(room.Participants))

	// Notify others in the room
	for participantID, conn := range room.Participants {
		if participantID != userID {
			s.logger.Info("Notifying existing participant of new user", "existing_user", participantID, "new_user", userID)
	...
	s.logger.Info("Sending room info to new user", "user_id", userID, "existing_participants", participantList)
