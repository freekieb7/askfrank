...
func (db *Database) CreateOAuthClient(ctx context.Context, params CreateOAuthClientParams) (OAuthClient, error) {
	...
	if _, err := db.Pool.Exec(ctx, `INSERT INTO tbl_oauth_client (id, name, secret, redirect_uris, is_public, allowed_scopes, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		client.ID, client.Name, client.Secret, client.RedirectURIs, client.IsPublic, client.AllowedScopes, client.CreatedAt, client.UpdatedAt); err != nil {
...
func (db *Database) ListOAuthClients(ctx context.Context, params ListOAuthClientsParams) ([]OAuthClient, error) {
	...
	var query strings.Builder
	query.WriteString(`SELECT id, name, secret, redirect_uris, is_public, allowed_scopes, created_at, updated_at FROM tbl_oauth_client WHERE 1=1`)
	var args []any
	// argNum := 1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&client.ID, &client.Name, &client.Secret, &client.RedirectURIs, &client.IsPublic, &client.AllowedScopes, &client.CreatedAt, &client.UpdatedAt); err != nil {
...
func (db *Database) GetOAuthClientByID(ctx context.Context, id uuid.UUID) (OAuthClient, error) {
	var client OAuthClient
	err := db.Pool.QueryRow(ctx, `SELECT id, name, secret, redirect_uris, is_public, allowed_scopes, created_at, updated_at FROM tbl_oauth_client WHERE id = $1`, id).Scan(
		&client.ID, &client.Name, &client.Secret, &client.RedirectURIs, &client.IsPublic, &client.AllowedScopes, &client.CreatedAt, &client.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
